...
# # training
vae.fit(x=x_train, y=x_train, epochs=200, validation_split=0.1, verbose=0)
...
# plt.plot(np.arange(48), np.squeeze(NL.descale(decoder(np.concatenate((sample, x_val[n:n+1, 48:]), axis=1))[:, :48], sc)), label='Generated Scenario')
...
# history = vae.fit(x=x_train, y=x_train, epochs=500, validation_split=0.1, verbose=0)
#
